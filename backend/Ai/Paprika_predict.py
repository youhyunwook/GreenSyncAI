...
def predict_control_values(df, models):
    ...
    try:
        ...
        opt_qty_div = opt_qty / 10   # 급수량을 10으로 나눠서 저장/출력
        results['WTSPL_QTY'] = round(opt_qty_div, 2)
        sys.stderr.write(f"예측 급수량(WTSPL_QTY, 10으로 나눔): {opt_qty_div:.2f}\n")
    except Exception as e:
...
def save_to_db(farm_id, control_values):
    try:
        ...
        cur.execute(insert_sql, (
            ...
            control_values['WTSPL_QTY']   # 10으로 나눈 값이 저장됨
        ))
        conn.commit()
        cur.close()
        conn.close()
